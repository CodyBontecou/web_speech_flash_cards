...
function manageInput(word, diagnostic, interim_span) {
    setText(word, diagnostic, interim_span);
    setTimeout(beep, 500, word);
    setTimeout(beginSpeechRecognition, 1000);
}
function setText(word, diagnostic, interim_span) {
    diagnostic.textContent = word;
    interim_span.textContent = '';
}
function beep(word) {
    var sound;
    if (words[iteration].spanish === word) {
...
function beginSpeechRecognition() {
    ...
    var diagnostic = document.getElementById('answer'), interim_span = document.getElementById('interim');
    handleIteration();
    var answer = words[iteration].english;
    var initial_word = document.getElementById('initial_word');
    if (!!initial_word) {
    ...
    recognition.onresult = function (event) {
        ...
        for (var i = event.resultIndex; i < event.results.length; ++i) {
            if (event.results[i].isFinal) {
                ...
                word = event.results[last][0].transcript.toLowerCase();
                manageInput(word, diagnostic, interim_span);
            }
            else {
...
function alterSiteForNonChromeUsers() {
    upgradeAlert();
    hideButton();
